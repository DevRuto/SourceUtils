...
namespace SourceUtils.MapExport.Bsp
{
    ...
    /// <summary>Ported from SourceUtils.WebExport.Bsp.ModelController, minus the HTTP layer.</summary>
    public static class BrushModelService
    {
        private static BspElement ConvertElement( ValveBspFile bsp, BspChild child )
        {
            return child.IsLeaf ? (BspElement) ConvertLeaf( bsp, child.Index ) : ConvertNode( bsp, child.Index );
        }

        private static BspNode ConvertNode( ValveBspFile bsp, int index )
        {
        ...
        private static BspLeaf ConvertLeaf( ValveBspFile bsp, int index )
        {
        ...
        public static BspModelPage GetPage( ValveBspFile bsp, int index )
        {
            var info = IndexService.GetPageLayout( bsp, bsp.Models.Length,
