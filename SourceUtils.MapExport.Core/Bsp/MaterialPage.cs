...
namespace SourceUtils.MapExport.Bsp
{
    ...
    /// <summary>Ported from SourceUtils.WebExport.Bsp.MaterialController (page listing), minus the HTTP layer.</summary>
    public static class MaterialPageService
    {
        public static MaterialPage GetPage( ValveBspFile bsp, int index )
        {
            ...
            for ( var i = 0; i < count; ++i )
            {
                ...
                foreach ( var prop in mat.Properties )
                {
                    ...
                    var texPath = Texture.GetTexturePathFromUrl( texUrl );
