...
namespace SourceUtils.MapExport
{
    ...
    public class Material
    {
            ...
            foreach ( var name in props.Keys )
            {
                ...
                switch ( name.ToLower() )
                {
                    ...
                    case "$nofog":
                        ...
                        break;
                    case "$alphatest":
                        if ( !ExportContext.Untextured ) mat.SetBoolean( "alphaTest", value );
                        break;
                    case "$translucent":
                    ...
                    case "$selfillumtint":
                        ...
                        break;
                    default:
                        if ( ExportContext.DebugMaterials )
                        {
        ...
        {
            var vmt = bsp == null
                ? ValveMaterialFile.FromProvider( path, ExportContext.Resources )
                : ValveMaterialFile.FromProvider( path, bsp.PakFile, ExportContext.Resources );
        ...
        public static Material CreateSkyMaterial( ValveBspFile bsp, string skyName )
        {
            ...
            for ( var face = 0; face < 6; ++face )
            {
                ...
                if ( face == 0 )
                {
                    ...
                    var tex = Texture.Get( bsp, Texture.GetTexturePathFromUrl( (Url) texProp.Value ) );
