...
{
    /// <summary>Matches each inner parser in sequence (the <c>+</c> combinator).</summary>
    public sealed class ConcatParser : Parser
    {
        ...
        {
            foreach ( var parser in inner ) Add( parser );
        }
        ...
        public override string ToString() =>
            string.Join( " ", Inner.Select( x => x is BranchParser ? $"({x})" : x.ToString() ) );
    }
}
